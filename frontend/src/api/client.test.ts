...
describe("API client", () => {
  ...
  it("does not attach Authorization to public status page requests", async () => {
    localStorage.setItem("projectops.auth.token", "secret-token");
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ ok: true }), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);

    await request("/api/v1/public/status-pages/abc123");

    expect(fetchMock).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/v1/public/status-pages/abc123",
      expect.objectContaining({
        headers: expect.not.objectContaining({ Authorization: expect.any(String) }),
      }),
    );
