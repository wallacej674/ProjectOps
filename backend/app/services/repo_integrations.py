...
class RepoIntegrationService:
    ...
    def attach_github_app_repo(
        self, db: Session, project_id: int, *, user_id: int, installation_id: int, repository_id: int, settings: Settings
    ) -> RepoIntegration:
        project_service.get_project(db, project_id)
        existing_repo = repo_integration_repository.get_by_project_id(db, project_id)
        repository = github_app_service.select_repository(
            db, settings, user_id=user_id, installation_id=installation_id, repository_id=repository_id
        )
        integration = repo_integration_repository.upsert_github_app_repo(
            db, project_id, installation_id=installation_id, repository=repository
        )
        from app.services.activity import activity_service
        activity_service.record_event(
            ...
            event_type="repository_replaced" if existing_repo else "repository_attached",
            event_category="repository",
            message="Private repository was replaced." if existing_repo else "Private repository was attached.",
            related_resource_type="repo_integration", related_resource_id=integration.id,
            metadata={"repo": repository["full_name"], "provider": "github", "connection_mode": "github_app"},
        )
        return integration
